...
const BLOCKED_HTTP_PATTERNS: readonly RegExp[] = [
  /\bfetch\s*\(/,
  /\brequests\.get\s*\(/,
  /\brequests\.post\s*\(/,
  /\bhttp\.get\s*\(/,
...
const FLOOD_COMMAND_PATTERNS: readonly RegExp[] = [
 ...
 */
const NARROWING_PATTERNS: readonly RegExp[] = [
...
export function installBashRoutingGuard(tools: ToolRuntime): () => void {
...
export function isFloodingSegment(segment: string): boolean {
  const value = segment.trim()
  if (value.length === 0) return false
  if (NARROWING_PATTERNS.some(pattern => pattern.test(value))) return false
  if (SAFE_COMMAND_PATTERNS.some(pattern => pattern.test(value))) return false
...
export function isSafeCurlWget(segment: string): boolean {
  ...
  if (isCurl && /\s(-o|--output)\s+(-|\/dev\/stdout)(\s|$)/.test(value)) return false
  if (isWget && /\s(-O|--output-document)\s+(-|\/dev\/stdout)(\s|$)/.test(value)) return false
  if (/\s(-v|--verbose|--trace)\b/.test(value)) return false
