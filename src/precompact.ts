 ...
 */
export function installPrecompactArchive(
  ctx: Context,
...
async function archive(
  ...
  for (const [layer, text] of grouped) {
    ...
    await client.callTool(
      'ctx_index',
      { content: segmentCjk(render(text)), source: `session/${key}/${layer}` },
...
function group(lines: readonly ArchivedLine[], maxChars: number): Map<LayerName, string> {
 ...
 */
export function classify(events: readonly SessionEventLike[]): ArchivedLine[] {
 ...
 */
export function statesAConcreteValue(text: string): boolean {
  return (
    /\d+\s*(ms|s|m|h|kb|mb|gb|%|个|次|行|条|秒|分|小时)/i.test(text) ||
...
function heading(event: SessionEventLike, layer: LayerName): string {
  const data = asRecord(event.data)
  if (layer === LAYERS.constraint) return `## [约束] 用户 (seq ${event.seq ?? '?'})`
...
export function textOf(event: SessionEventLike): string {
  ...
  if (typeof content === 'string') return clip(content, 4_000)
...
function sessionId(session: SessionLike): string {
