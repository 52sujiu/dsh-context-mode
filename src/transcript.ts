 ...
 */
export function isCheckpointEvent(event: TranscriptEventLike): boolean {
  const source = sourceOf(event)
  if (source !== undefined && source.kind === 'plugin' && source.plugin === 'compact') return true
 ...
 */
function toolNames(events: readonly TranscriptEventLike[]): Map<string, string> {
...
function transcriptLine(
  ...
  if (event.type === 'tool/result') return toolLine(event, seq, options, names)
  ...
  if (event.type === 'user/message') {
    if (isCheckpointEvent(event)) return undefined
    if (isInjected(event)) return undefined
    return userLine(event, seq, options)
  }
  if (event.type === 'assistant/message') {
    if (options.keepAssistant === false) return undefined
...
export function clipNotice(
  ...
  return (
    `[... ${dropped} of ${total} chars elided from seq ${seq}; ` +
...
function messageText(event: TranscriptEventLike): string {
  const data = asRecord(event.data)
...
function firstCallId(content: unknown): string | undefined {
...
function toolNameFor(
  ...
  if (data !== undefined && typeof data.name === 'string' && data.name.length > 0) return data.name
...
function isInjected(event: TranscriptEventLike): boolean {
  const text = messageText(event).trimStart()
  return (
    text.startsWith('<current_runtime_context') ||
...
function sourceOf(event: TranscriptEventLike): { kind?: string; plugin?: string } | undefined {
  const data = asRecord(event.data)
