...
import { buildNodeCommand, type HookAdapter, type PlatformId, isInProcessPluginPlatform } from "./platform/types.js";
import { detectPlatform, getSessionDirSegments } from "./platform/detect.js";
import { parseCodexContextModePluginRoot } from "./platform/dsh.js";
...
async function getDiagnosticAdapter(): Promise<HookAdapter | null> {
  ...
  try {
    const { getAdapter } = await import("./platform/detect.js");
...
async function main() {
  ...
  try {
    const { detectPlatform, getAdapter } = await import("./platform/detect.js");
    const clientInfo = server.server.getClientVersion();
    const signal = detectPlatform(clientInfo ?? undefined);
